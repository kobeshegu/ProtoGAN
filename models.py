...
class ChannelAttention(nn.Module):
    def __init__(self, in_planes, ratio=16):
        ...
        self.fc = nn.Sequential(nn.Conv2d(in_planes // 2  , in_planes // 64, 1, bias=False),
                                nn.BatchNorm2d(in_planes // 64),
                                nn.ReLU(),
                                nn.Conv2d(in_planes // 64, in_planes // 2 , 1, bias=False))
    ...
    def forward(self, x):
       ...
       # print(x.shape)
        #print(x)
       # x.resize(x.shape[0],x.shape[1]*2,x.shape[2],x.shape[3]xingbu)
        avg_pool = self.avg_pool(x)
       # print(avg_pool.shape)
       # print("FXXXXXXXXXXXXXXXXXXXXXXXXXXXXk")
...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=100, nc=3, im_size=1024):
        ...
        self.catten = ChannelAttention(im_size)
    ...
    def forward(self, input):
        ...
        #CA & SA
#        feat_catten = self.catten(feat_32) * feat_32
       # feat_satten = self.satten(feat_catten) * feat_catten
        # change feat_satten to feat_32
...
class Discriminator(nn.Module):
    ...
    def forward(self, imgs, label):
        ...
        if label=='real':    
            ...
            #print(torch.cat([rf_0, rf_1]))

            return torch.cat([rf_0, rf_1]) , [rec_img_big, rec_img_small, rec_img_part], part, feat_out, feat_out_mean

        #print(torch.cat([rf_0,rf_1]))
        # print(torch.cat([rf_0, rf_1]).mean())
        # print("************fake???")
        return torch.cat([rf_0, rf_1]), feat_out, feat_out_mean, feat_out_var
